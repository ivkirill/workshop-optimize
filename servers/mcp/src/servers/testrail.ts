...
function getResults(runId: string) {
  ...
  return {
    ...
    source: 'mirrors `npm run scenario:verify` (the active scenario quality gate)',
    gate: gate ?? { note: 'gate not run yet — run `npm run scenario:verify`' },
    ...
    results: cases.map((c) => ({
      ...
      title: c.title,
      ...statusFor(c.id),
      comment: known ? `Reflected from quality gate (tests=${gate?.tests}).` : 'Run the quality gate to populate results.',
      elapsed: '1s',
      metadata: metadataBloat(c.id),
    })),
  };
