...
@Component({
  ...
  template: `
    ...
    @switch (state.status) {
      ...
      @case ('empty') {
        <p class="pl-state">No orders match.</p>
      }
      @case ('results') {
        <table class="pl-table">
          <thead>
            <tr><th>Order</th><th>Customer</th><th>Location</th><th>Status</th><th>Total</th></tr>
          </thead>
          <tbody>
            @for (o of state.page.items; track o.id) {
              <tr>
                ...
                <td>{{ o.customer }}</td>
                <td>{{ o.location }}</td>
                <td><span class="pl-badge">{{ o.status }}</span></td>
                <td>{{ '$' + o.total }}</td>
              </tr>
            }
          </tbody>
      ...
      @default {
        <p class="pl-state">Type to search orders.</p>
      }
    }
...
export class OrdersPageComponent implements OnInit, OnDestroy {
  ...
  onStatus(event: Event): void {
    ...
    this.controller.setStatus(value ? (value as OrderStatus) : undefined);
  }

  onSort(event: Event): void {
    this.controller.setSort((event.target as HTMLSelectElement).value as OrderSort);
  }
